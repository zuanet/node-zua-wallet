...
import {ZuaAPI, ApiError} from './api';
...
const SompiPerZua = 100_000_000

// MaxSompi is the maximum transaction amount allowed in sompi.
const MaxSompi = 4_961_000_000 * SompiPerZua
...
class Wallet extends EventTargetImpl {
	...
	static networkTypes: Object = {
		zua: { port: 42110, network: 'zua', name : 'mainnet' },
		zuatest: { port: 42210, network: 'zuatest', name : 'testnet' },
		zuasim: { port: 42510, network: 'zuasim', name : 'simnet' },
		zuadev: { port: 42610, network: 'zuadev', name : 'devnet' }
	}
	...
	constructor(privKey: string, seedPhrase: string, networkOptions: NetworkOptions, options: WalletOptions = {}) {
		super();
		this.logger = CreateLogger('ZuaWallet');
		this.api = new ZuaAPI();
		//@ts-ignore
	...
	*/
	...
	composeTx({
		...
		try {
			const tx: zuacore.Transaction = new zuacore.Transaction()
				.from(utxos)
				.to(toAddr, amount)
				.setVersion(0)
				.fee(fee)
	...
	*/
	...
	async buildTransaction(txParamsArg: TxSend, debug = false): Promise < BuildTxResult > {
		...
		const ts_3 = Date.now();
		const inputs: RPC.TransactionInput[] = tx.inputs.map((input: zuacore.Transaction.Input) => {
			if (debug || this.loggerLevel > 0) {
		...
		const ts_4 = Date.now();
		const outputs: RPC.TransactionOutput[] = tx.outputs.map((output: zuacore.Transaction.Output) => {
			return {
